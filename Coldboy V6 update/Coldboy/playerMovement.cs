...
public class playerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            playerAnimator.SetInteger("activeState", 0); //no accessory
        }
        else if (Input.GetKey(KeyCode.Alpha2) && player.checkGun())
        {
            playerAnimator.SetInteger("activeState", 1); //gun held
        }
        else if (Input.GetKey(KeyCode.Alpha3) && player.checkJet())
        {
            playerAnimator.SetInteger("activeState", 2); //jetpack on
        }
        ...
        if (Input.GetKeyDown(KeyCode.W) && (jetGrounded) && playerAnimator.GetInteger("activeState") == 2)
        {
            if (player.getEnergy() >= 10)
            {
                player.UseEnergy(10);
                Debug.Log("Energy: " + player.getEnergy());
                r2d.velocity = new Vector2(r2d.velocity.x, jumpHeight * 1.25f);
            }
        }
